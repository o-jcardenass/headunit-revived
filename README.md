...
| `bring-up-status-pill-and-poke-readiness` | **ROUND 2 DONE** | bring-up-status-pill-and-poke-readiness-round2-results.md; same SHA `ef66abbf` re-run in full, gate 1616/0 (identical apk md5). W1/W1b/W2/W2b/W3, P1/P2/P4, D1 PASS, reproducing round 1. **P3 FAIL again** (mode-3 pill latches on WAKING_PHONE), same mechanism. D1 grace ref 9807 ms (round 1: 9420 ms), both < 15000. P6 INCONCLUSIVE (no USB host). **D2/D3 INCONCLUSIVE this round** (D-HU's `WirelessServer` bound IPv6-only twice, phone joined the P2P network but its IPv4 connect was refused; correlated with D-HU's station WiFi holding a concurrent primary connection). New finding: a stray bonded intercom ("FX Plus") with an Uncategorized Bluetooth class gets misclassified as a phone by `DriverCandidatePolicy` because it advertises the Audio-Gateway record — pre-existing gap, not from this branch, follow-up recommended. |
