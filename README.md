...
| `bring-up-status-pill-and-poke-readiness` | **ROUND 4 QUEUED** | bring-up-status-pill-and-poke-readiness-round4-brief.md; candidate `7f439d4e` (3 commits past round 3's `fe8b91e2`), gate 1628/0. Round 3's P1/P3 verdicts were the brief's wording: the retreat stays and P1/P3 are reworded around it. WB1 (WiFi button shows no pill) fixed: every connect is pill-first, the overlay takes over when the phone answers, the "Connecting to" toast is gone. FX Plus (Uncategorized class with a gateway record) is now UNKNOWN. D3 re-measured with a scripted +3 s toggle. Round 2's D2/D3 IPv6 reading is withdrawn: D-POCO held its own P2P group after a force-stop, same as round 3's void D2; a role swap now gets `headunit://exit` first. |
