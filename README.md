...
| `bring-up-status-pill-and-poke-readiness` | **ROUND 3 DONE** | bring-up-status-pill-and-poke-readiness-round3-results.md; candidate advanced to `fe8b91e2` (2 commits past round 1/2's `ef66abbf`), gate 1626/0. **P1 now FAIL, intermittent** (1 FAIL/1 PASS): the P3 fix's new `retreat()` call regresses P1's rank-monotonicity rule on any bring-up needing more than one poke round. **P3's original latch is gone** (pill now cycles WAKING_PHONE/WAITING_FOR_PHONE over 246s) but never reaches `ARMED`, so still FAIL on the brief's literal wording. P2 PASS despite pill growing to 352×105 (5GHz-toast-into-pill line). P4 PASS, unaffected. P5's toast-over-pill finding is resolved. **New finding WB1: the WiFi button's manual connect shows zero pill/stage lines** (OVERLAY UI mode suppresses it by design) — pre-existing on `ef66abbf` too, not a regression, but open. **Addendum: Part C (D1/D2/D3) run after all** — D1 PASS (9253ms, 3rd consistent measurement), D2 PASS, D3 PASS but near the 15000ms ceiling (14803ms, timing landed at +8s not the brief's +3s). Round 2's D2/D3 IPv6-bind issue did not recur. Part B (W1-W3) and P6 still not re-run (unaffected by either commit). |
